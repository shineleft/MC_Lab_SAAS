...
def run_demo():
    ...
    safe_distance = 3  # Minimum safe distance to avoid obstacles
    avoid_speed = 1.5    # Speed when avoiding obstacles
    avoid_angle = math.pi/2    # Angle to turn when avoiding obstacles

    
    while not rospy.is_shutdown():
        # Get the sonar reading (distance to the closest obstacle)
        sonar_distance = robot.get_sonar()

        if sonar_distance < safe_distance:
            # Obstacle detected: reactive avoidance
            print(f"{robot_name} detected obstacle at distance = {sonar_distance}. Avoiding...")
            
            # Reactive avoidance: turn and reduce speed
            velocity = avoid_speed
            angle = avoid_angle  # Positive angle turns left; adjust as needed
            
            robot.set_speed_angle(velocity, angle)
            rospy.sleep(0.3)
            continue  # Skip further processing and continue avoidance
